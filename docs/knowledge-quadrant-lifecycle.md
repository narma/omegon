...
open_questions:
  - "Should the readiness score be a hard gate (node CAN'T transition to decided until readiness > threshold) or advisory (dashboard shows the score, operator/agent decides when to proceed)?"
  - "Should assumptions be a first-class section in the design tree markdown (like Open Questions and Decisions) or tracked as a special type of open question with an 'assumption' tag?"
issue_type: feature
priority: 1
---
